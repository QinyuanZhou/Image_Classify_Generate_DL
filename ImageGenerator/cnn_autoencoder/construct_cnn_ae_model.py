...
class Conv_AE(object):
    ...
    def middle_code(self): # n x 12 x 12 x 16
        with tf.name_scope('Middle_Code'):
            with tf.name_scope('M_Layer1'):
                ...
                epsilon = 0.000001
                self.m_code = tf.nn.batch_normalization(self.m_code, m_mean, m_var, scale, shift, epsilon)
    ...
    def decoder(self, rand_code): # n x 6 x 6 x 16
        with tf.name_scope('Decoder_Layer'):
            ...
            with tf.name_scope('Decoder_Layer4'):
                ...
                recon = tf.nn.conv2d(d_conv4, filter=tf.Variable(tf.truncated_normal([3, 3, 64, 3], stddev=0.1, dtype=tf.float32), name='D_weight4'), strides=self.c_stride, padding='SAME') # n x 96 x 96 x 3
        return recon
    ...
    #     sess.run(init_var)
    #     # print('Activate train_net function')
