...
class Class_Pekoman_Net(object):
    ...
    def build_net(self, input_images, image_channels=3):
        with tf.name_scope('Conv_Layer'):
            ...
            with tf.name_scope('conv3'):
                ...
                pool3 = tf.nn.max_pool(relu3, ksize=[1, 1, 1, 1], strides=[1, 1, 1, 1],
                                       padding='SAME')
                image_shape = input_images.shape.as_list()
                flat_size = (image_shape[1]) * (image_shape[2]) * 8
                print(image_shape)
            with tf.name_scope('dense1'):
                pool3_flat = tf.reshape(pool2, [-1, flat_size])
    ...
    def train_model(self, input_images, input_labels, image_channels):
        ...
        with tf.name_scope('Loss'):
            loss = tf.nn.softmax_cross_entropy_with_logits(labels=input_labels, logits=y_hat)
